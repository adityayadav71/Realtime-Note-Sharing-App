...
mongoose
  ...
  .then(() => {
    if (process.env.NODE_ENV === "DEV") console.log("DB connection successful! 🖥️");
  });

const port = process.env.PORT || 5000;
const server = app.listen(port, () => {
  if (process.env.NODE_ENV === "DEV") console.log(`App running on port ${port} ✅`);
});

function serverTime() {
  const time = new Date(Date.now()).getHours().toString().padStart(2, "0") + ":" + new Date(Date.now()).getMinutes().toString().padStart(2, "0");
  return time;
}
...
io.on("connection", (socket) => {
  ...
  socket.on("join-room", (user, room, reloaded) => {
    try {
      socket.join(room.roomId, () => {
        console.log(`The user: ${user?.userId} has joined the room successfully.`);
      });
  ...
  socket.on("NOTE_UPDATED", (note, roomId, author) => {
    socket.to(roomId).emit("NOTE_UPDATED", note, author);
  });
});
